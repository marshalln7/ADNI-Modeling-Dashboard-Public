...
def merge_profile_variables():
    ...
    raw_files_list = os.listdir(working_directory + "/Datasets/Raw Data Files")
    ...
    profile_variables = pd.read_excel("Datasets/Profile Variables.xlsx")
    ...
    def has_conflicts(group):
        ...
        are_conflicts = False #Changes to True if it finds anything
        for col in group.columns:
            if col != 'PHASE':  # Skip the phase column
    ...
    #group together the rows that take no new information from each other, function is more to check how many aren't and why
    merged_dataframe = merged_dataframe.groupby(['RID', "PHASE"], group_keys=False).apply(combine_rows)  
    ...
    #if there are variables that you know you'll want to aggregate a certain way when they come up, put them here
    ...
    #gets rid of irrelevant aggregation functions listed above
    aggregation_functions = {col: func for col, func in aggregation_functions.items() if col in list(variables_to_merge.values())}
    ...
    #aggregate
    aggregated_dataframe = merged_dataframe.groupby(['RID','PHASE'], as_index=False).agg(aggregation_functions)
    ...
    #gets rid of irrelevant aggregation functions listed above
    variable_list = [variable for table_list in variables_to_merge.values() for variable in table_list]
    remapping_dictionaries = {col: func for col, func in remapping_dictionaries.items() if col in variable_list}
    ...
    if "DX_bl" in variables_to_merge.values():
    ...
    if os.name == 'nt':
        print("You're on Windows! Opening merged excel file...")
