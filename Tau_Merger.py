...
raw_data = pd.read_csv("Datasets/Raw Data Files/UCBERKELEY_TAUPVC_6MM - UC Berkeley - Tau PET PVC 6mm Res analysis [ADNI2,3,4].csv")
...
#pulls everything I put in there out to make a dictionary for translating VISCODEs to VISMONTHs, not currently used because turns out the PET scans were taken wayyy later than the codes indicate!
#translation_dictionary = {row["VISCODE"]: row["VISMONTH"] for index, row in visit_codes.iterrows()}
#raw_data["VISMONTH"] = raw_data["VISCODE"].map(translation_dictionary)

#filter out the odd tracer
raw_data = raw_data[raw_data["TRACER"] == "FTP"]

#keep only the columns that we need
columns_list = list(raw_data.columns)
indexers_to_delete = ["MONTH", "BASELINEDATE", "LONIUID", "VISCODE", "SITEID", "PTID", "SCANDATE", "PROCESSDATE", "TRACER", "TRACER_SUVR_WARNING", "update_stamp"]
...
new_filename = "Tau Variables " + date_string + ".xlsx"
